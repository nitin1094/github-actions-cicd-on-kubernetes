...
In real production you would use Kubernetes (or ECS, or Fargate, or Cloud Run) instead of a single EC2 — that is the subject of the Kubernetes guide. But every production engineer started by learning to deploy onto a single VM, and you should too.
...
Go to `github.com/nitin1094/github-actions-cicd-on-kubernetes` and click "Fork." This creates `github.com/<your-username>/github-actions-cicd-on-kubernetes`.

Then clone *your fork* locally:

```bash
git clone git@github.com:<your-username>/github-actions-cicd-on-kubernetes.git
cd github-actions-cicd-on-kubernetes
```
...
**4. Replace the SSH deploy with Kubernetes.** This is what the Kubernetes guide covers. Same app, same CI workflow producing the same images — but instead of `appleboy/ssh-action`, the CD workflow runs `kubectl apply` against an EKS / minikube / kind cluster. Suddenly you have:
